...
def save_to_checkpoint(save_path, epoch, model, optimizer, scheduler=None, verbose=True, info='', AP=0.0, model_decoder=None):
    ...
    if save_path is not None:
        if model_decoder is None:
            ...
            torch.save(checkpoint, '{}/epoch_{}_{}.pt'.format(save_path, epoch, info))
        else:
            checkpoint = {'epoch': epoch,
                          'AP': AP,
                          'model_encoder': model.state_dict(),
                          'model_decoder': model_decoder.state_dict(), # added
                          'optimizer': optimizer.state_dict(),
                          'scheduler': d_sche
                          }
            torch.save(checkpoint, '{}/epoch_{}_{}.pt'.format(save_path, epoch, info))
...
def reshape_bbox(bbox_list, device):
    ...
    for j in range(len(bbox_list)):
        ...
        new_bbox_list.append(torch.stack(bboxes))
    new_bbox_list = torch.stack(new_bbox_list)
    new_bbox_list = new_bbox_list.permute(1,0,2,3)

    return new_bbox_list


def reshape_bbox_original(bbox_list, device):
    new_bbox_list = []
    for j in range(len(bbox_list)):
        raw_bboxes = bbox_list[j]
        B = torch.stack(raw_bboxes).type(torch.FloatTensor).to(device, non_blocking=True)
...
def reshape_bbox_original_2(bbox_list, device):
    new_bbox_list = []
    for j in range(len(bbox_list)):
        raw_bboxes = bbox_list[j]
        B = torch.stack(raw_bboxes).type(torch.FloatTensor).to(device, non_blocking=True)
        ...
        for i in range(B.shape[1]):
            bboxes.append(B[:, i].view(1, 4))
        new_bbox_list.append(torch.stack(bboxes))
    return torch.stack(new_bbox_list).squeeze(2)
