...
def read_csv_titan(anns_dir, vid):
    ...
    df = pd.read_csv(os.path.join(anns_dir, vid, 'label2d3d.csv'))
        ...
        for i in range(n):
            ...
            for j in range(flag, len(ped_info_raw)):
                if ped_info_raw[j][1] == pids[i]:
                    ...
                    box = list(map(round, [ele[3], ele[2], ele[3] + ele[5], ele[2] + ele[4]]))
                    box = list(map(float, box))
                    if ele[6] in ["Moving", "Crossing the road"]:
                        action = 1
                    elif ele[6] in ["Stopped", "Waiting to cross"]:
                        action = 0
                    else:
                        action = -1
...
def save_file(anns_dir, vids) -> dict:

    for vid in tqdm.tqdm(sorted(vids)):
        frames = sorted(glob(os.path.join(anns_dir, vid, '*.png')))
        path_3d_total = sorted(glob(os.path.join(anns_dir, vid, 'label3d*.txt')))
        path_2d_total = sorted(glob(os.path.join(anns_dir, vid, 'label2d*.json')))
        df_total = pd.DataFrame(columns=['frames', 'obj_track_id', 'top', 'left', 'height', 'width', 'action'])
        for num, path_2d in enumerate(path_2d_total):
            f = open(path_2d)
            data = json.load(f)
            data_ped = data['Pedestrian']
            if data_ped == {}:
                continue
            path_3d = path_3d_total[num]
            data_3d = pd.read_csv(path_3d, sep=",", header=0, usecols = ['labels',' track_id',' intended_actions'])
            data_3d = data_3d.rename(columns={' track_id': 'track_id', ' intended_actions': 'intended_actions'})
            df = data_3d[data_3d['labels'] == 'Pedestrian']
            for i in data_ped.keys():
                if i in df['track_id'].to_list():
                    action = df[df['track_id'] == i]['intended_actions'].item()
                    frame = path_2d.split('/')[-1].split('_')[-1].split('.')[0]
                    top = data_ped[i]['box']['top']
                    left = data_ped[i]['box']['left']
                    height = data_ped[i]['box']['height']
                    width = data_ped[i]['box']['width']
                    df_total = df_total.append({'frames': frame, 'obj_track_id': i, 'top': top, 'left': left, 'height': height, 'width': width, 'action': action}, ignore_index=True)
        df_total.to_csv(os.path.join(anns_dir, vid, 'label2d3d.csv'), index=False)
...
def build_ped_dataset_titan(anns_dir, split_vids_path, image_set="all", verbose=False) -> dict:
    ...
    vids = os.listdir(anns_dir)
    #save_file(anns_dir, vids) #per salvare i file csv
    ...
    def extract_non_trans(self, fps=10, max_frames=None, max_samples=None, verbose=False):
        ...
        for idx in ids:
            ...
            if key is not None:
                ...
                samples[action_type][key]["source"] = "LOKI"
