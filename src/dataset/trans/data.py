...
def balance_frame_sample(samples, seed=99, balancing_ratio=1, verbose=True, memory_flag=False) -> dict:
    """
    ...
    for k in ids:
        if samples[k]['source'] == "JAAD":
            ...
            ids_t.append(k)
        elif samples[k]['source'] == "LOKI":
            ids_l.append(k)
    # balance positive and negative samples within each dataset
    for _ids in [ids_j, ids_p, ids_t, ids_l]:
        if len(_ids) == 0:
            continue
...
def extract_pred_sequence(trans, non_trans=None, pred_ahead=0, balancing_ratio=None,
                          bbox_min=0, max_frames=None, seed=None, neg_in_trans=True, verbose=False, memory_flag=False) -> dict:
    """
    ...
    for idx in ids_trans:
        ...
        speed = copy.deepcopy(trans[idx]['speed'])
        if "behavior" in list(trans[idx].keys()):
            behavior = copy.deepcopy(trans[idx]['behavior'])
        else:
            behavior = []
    ...
    if non_trans is not None:
        ...
        for idx in ids_non_trans:
            ...
            speed = copy.deepcopy(non_trans[action_type][idx]['speed'])
            if "behavior" in list(non_trans[action_type][idx].keys()):
                behavior = copy.deepcopy(non_trans[action_type][idx]['behavior'])
            else:
                behavior = []
    ...
    if balancing_ratio is not None:
        samples = balance_frame_sample(samples=samples, seed=seed, balancing_ratio=balancing_ratio, verbose=verbose, memory_flag=memory_flag)
    else:
        print('No balancing ratio is given. No balancing is performed.')
    if memory_flag:
