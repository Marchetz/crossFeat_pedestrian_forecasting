...
def get_pedb_info_jaad(annotations, vid):
    ...
    for idx in ids:
        ...
        for i in range(len(frames)):
            spe = dataset[vid]['vehicle_annotations'][frames[i]]
            speed_list.append(spe)
        ...
        pedb_info[idx]['speed'] = speed_list
        # attribute vector
        atr_vec = [0, 0, 0, 0, 0, 0]
...
def pedb_info_clean_jaad(annotations, vid) -> dict:
    ...
    for idx in pids:
        ...
        for i in range(len(full_occ)):
            ...
            pedb_info[idx]['speed'][full_occ[i]] = None
        ...
        pedb_info[idx]['speed'] = list(filter(filter_None, pedb_info[idx]['speed']))
    ...
    for vid in vids:
        ...
        for idx in pids:
            if len(pedb_info[idx]['action']) > 0:
                ...
                pedb_dataset[idx]["speed"] = pedb_info[idx]["speed"]
...
class JaadTransDataset:
    ...
    def extract_non_trans(self, fps=30, max_frames=None, max_samples=None, post_frames=0, verbose=False):
        dataset = self.dataset
        ids = list(dataset.keys())
