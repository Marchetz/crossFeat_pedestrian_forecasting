...
def read_csv_titan(anns_dir, vid):
    ...
    path_imu = 'imu_data'
    #rad csv without header

    df_imu = pd.read_csv(os.path.join(path_imu, vid, 'synced_sensors.csv'), header=None)
    ...
    frames_acc = {}
    frame_imu = df_imu.iloc[:, 1].values
    acc = df_imu.iloc[:, 3].values
    for i_f, f in enumerate(frame_imu):
        frames_acc[f.split('/')[-1].split('.')[0]] = acc[i_f]

    return video_number, ped_info_raw, pids, frames_acc
...
def convert_anns_titan(anns_dir, vids) -> dict:
    ...
    for vid in tqdm.tqdm(vids):
        video_number, ped_info_raw, pids, acc = read_csv_titan(anns_dir, vid)
        n = len(pids)
        flag = 0
        for i in range(n):
            idx = f"ped_{video_number}_{i + 1}"
            ...
            for j in range(flag, len(ped_info_raw)):
                if ped_info_raw[j][1] == pids[i]:
                    ...
                    anns[idx]['speed'].append(acc[str(t).zfill(6)])
...
class TitanTransDataset:
    ...
    def extract_trans_history(self, mode="GO", fps=10, max_frames=None, post_frames=0, verbose=False) -> dict:
        ...
        for idx in ids:
            ...
            next_transition = copy.deepcopy(dataset[idx]["next_transition"])
            speed = copy.deepcopy(dataset[idx]['speed'])
            for i in range(len(frames)):
                key = None
                if mode == "GO":
                    if next_transition[i] == 0 and action[i] == 1:
                ...
                if key is not None:
                    ...
                    samples[key]['speed'] = speed[i:t:-step]
                    samples[key]['speed'].reverse()

                    samples[key]['frame_total'] = frames
                    samples[key]['bbox_total'] = bbox
                    samples[key]['action_total'] = action
                    samples[key]['attributes'] = [-1,-1,-1,-1,-1,-1]
    ...
    def extract_non_trans(self, fps=10, max_frames=None, max_samples=None, verbose=False):
        ...
        for idx in ids:
            ...
            if key is not None:
                ...
                samples[action_type][key]['speed'] = speed[-1:t:-step]
                samples[action_type][key]['speed'].reverse()

                samples[action_type][key]['frame_total'] = frames
                samples[action_type][key]['bbox_total'] = bbox
                samples[action_type][key]['action_total'] = action
